...
const source={schema_version:2,horizon:'weekly',rank_scope:'position',...weekly,source:'FantasyPros export',scoring_format:'ppr',snapshot_at:'2026-09-10T10:00:00Z',players:[{player_id:'a',position:'RB',ecr:3,projected_fpts:12.3}]};
...
assert.equal(prepare(source,weekly,now).get('a').projected_fpts,undefined);
for(const change of [{horizon:'ros'},{rank_scope:'overall'},{schema_version:1}]) assert.throws(()=>prepare({...source,...change},weekly,now));

...
- **Amendment (2026-07-10, Plan 2 Task 8 — snap % excluded in v1):** Spec originally listed snap % among sequence usage features. A join spike (`nflreadpy.load_snap_counts` keyed on `pfr_player_id` → `nflreadpy.load_players` pfr_id/gsis_id crosswalk → canonical weekly frame on `player_id`/`season`/`week`) measured per-season match rate for QB/RB/WR/TE player-weeks, gated at ≥90% every season 2012–2025 to ship:
  ...
  - **Amendment (2026-07-10, snap-count usage feature shipped with NaN-for-2012 semantics):** User approved shipping `snap_pct` (offense snap share, same join methodology and measurement table above) with the same NaN convention already used for `target_share`: unmatched/unavailable rows (all of season 2012) stay NaN rather than being imputed, and NaN flows natively through the scaler (`fit_scaler`/`apply_scaler` in `src/ffmodel/model/dataset.py` are already NaN-aware — mean/std computed over non-NaN entries, NaN inputs zero-filled after scaling). This supersedes the ≥90%-every-season gate above: that gate is no longer a shipping blocker for this feature, since the 2012 gap is a known, accepted, and clearly-labeled data limitation rather than a join defect. The per-season measurement table is retained above for the record — it remains accurate (2013–2025 all ≥99.7%; 2012 is 0.00% because nflverse has no source data for that season, not a crosswalk failure). `snap_pct` is implemented in `merge_snap_pct()` (`src/ffmodel/data/pull.py`), added to `LAG_STATS` (`src/ffmodel/data/features.py`) and `SEQ_FEATURES` (`src/ffmodel/model/dataset.py`); the raw same-week value is deliberately excluded from `feature_columns()`'s allow-list (only `lag4_`/`lag8_` aggregates, context features, and position dummies qualify), matching every other same-week stat.
